<script lang="ts">
  ...
  const processInput = () => {
    ...
    const nonEmptyRows = rows.filter(row => row.length > 0).map(row => row.trim())
    ...
    for (const currentRow of nonEmptyRows){
      ...
      const durationInMinutes = endTimeHours*60+endTimeMinutes-startTimeHours*60-startTimeMinutes
      ...
      const rowOutput = `${currentRow} DURATION: ${minutesToTimeString(durationInMinutes)} CUMULATIVE: ${minutesToTimeString(minuteSums[key])}`
